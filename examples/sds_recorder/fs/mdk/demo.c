...
#endif
#ifndef REC_BUF_SIZE_MAGNETOMETER
#define REC_BUF_SIZE_MAGNETOMETER       8192U
#endif
#ifndef REC_IO_THRESHOLD_ACCELEROMETER
#define REC_IO_THRESHOLD_ACCELEROMETER  512
#endif
#ifndef REC_IO_THRESHOLD_MAGNETOMETER
#define REC_IO_THRESHOLD_MAGNETOMETER   512
#endif
...
static sensorId_t sensorId_magnetometer;

// Sensor configuration
static sensorConfig_t *sensorConfig_accelerometer;
static sensorConfig_t *sensorConfig_magnetometer;

// Recorder identifiers
static sdsRecId_t recId_accelerometer = NULL;
static sdsRecId_t recId_magnetometer  = NULL;

// Recorder buffers
static uint8_t recBuf_accelerometer[REC_BUF_SIZE_ACCELEROMETER];
static uint8_t recBuf_magnetometer[REC_BUF_SIZE_MAGNETOMETER];
...
static __NO_RETURN void read_sensors (void *argument) {
  ...
  for (;;) {
    if (close_flag == 0U) {
      ...
      if (sensorGetStatus(sensorId_magnetometer).active != 0U) {
        num = sizeof(sensorBuf) / sensorConfig_magnetometer->sample_size;
        num = sensorReadSamples(sensorId_magnetometer, num, sensorBuf, sizeof(sensorBuf));
        if (num != 0U) {
          buf_size = num * sensorConfig_magnetometer->sample_size;
          num = sdsRecWrite(recId_magnetometer, timestamp, sensorBuf, buf_size);
          if (num != buf_size) {
            printf("%s: Recorder write failed\r\n", sensorConfig_magnetometer->name);
          }
        }
      }
    } else {
...
static void recorder_start (void) {
  ...
  // Magnetometer
  // Open Recorder
  recId_magnetometer = sdsRecOpen("Magnetometer",
                                  recBuf_magnetometer,
                                  sizeof(recBuf_magnetometer),
                                  REC_IO_THRESHOLD_MAGNETOMETER);
  sensorEnable(sensorId_magnetometer);
...
static void recorder_stop (void) {
  ...
  if ((flags & osFlagsError) == 0U) {
    ...
    sensorDisable(sensorId_magnetometer);
    // Close Recorder
    sdsRecClose(recId_magnetometer);
    recId_magnetometer = NULL;
...
static __NO_RETURN void demo (void *argument) {
  ...
  sensorId_magnetometer = sensorGetId("Magnetometer");

  // Get sensor configuration
  sensorConfig_accelerometer = sensorGetConfig(sensorId_accelerometer);
  sensorConfig_magnetometer  = sensorGetConfig(sensorId_magnetometer);
